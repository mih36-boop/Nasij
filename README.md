...
Web Application
Nasij was implemented as an interactive web application using Streamlit.
The application contains three main sections:
1. Report an Issue
2. Submit a Suggestion
3. Municipality Dashboard
In the Report an Issue section, citizens can upload an image, enter the location of the problem, and provide additional details. The trained YOLOv8 model analyzes the image and identifies supported infrastructure issues. When an issue is detected, the application creates a maintenance ticket containing the detected issue, confidence score, location, submission time, and status.
In the Submit a Suggestion section, citizens can enter feedback in English or Arabic. The NLP pipeline processes the text and assigns it to one of the five civic categories.
The Municipality Dashboard combines the results from both pipelines and presents infrastructure reports, citizen suggestions, topic frequencies, top citizen concerns, representative feedback, and maintenance tickets in one interface.

Deployment
The Nasij application was deployed using Streamlit Community Cloud.
The project code, trained model, NLP notebook, Computer Vision notebook, and deployment files are stored in the GitHub repository.
The trained YOLO model is loaded directly into the Streamlit application for image analysis, while the multilingual Sentence Transformer is used to process citizen suggestions.
...
Limitations
Nasij is currently an MVP and has several limitations.
The Computer Vision dataset is imbalanced, particularly for underrepresented classes such as water leaks. The model may also perform differently under lighting conditions or environments that differ from the training data.
The streetlight class detects the presence of a streetlight but does not determine whether it is functioning correctly.
The NLP evaluation dataset contains only 30 suggestions and does not represent the full variety of Lebanese language use, including Arabizi, spelling variations, mixed-language sentences, and different dialect expressions.
The current application also stores reports and suggestions temporarily using Streamlit session state rather than a persistent database.

Future Work
Future versions of Nasij could include:
- Persistent database storage
- Citizen and municipality accounts
- Ticket status tracking
- GPS-based issue locations
- Interactive maps
- Duplicate report detection
- Larger and more balanced Lebanese Computer Vision datasets
- Larger Lebanese Arabic and Arabizi NLP datasets
- Issue severity estimation
- Urgency and sentiment analysis
- Automatic citizen notifications
- Integration with real municipal systems

Conclusion
Nasij demonstrates how Computer Vision and Natural Language Processing can be combined to organize citizen feedback within a single civic platform.
Infrastructure images are transformed into structured maintenance tickets, while written suggestions are classified and aggregated to identify community priorities.
The municipality dashboard brings both sources of information together, helping transform scattered citizen input into structured information that is easier to understand, prioritize, and act upon.
